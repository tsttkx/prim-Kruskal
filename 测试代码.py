...
class Ui_MainWindow(object):
    ...
    def write(self):
        k=0
        with open("测试文件1.txt",'a',encoding='utf-8') as f:
            ...
            # f.write('\n')
            for i in range(self.listWidget_2.count()):
                now=self.listWidget_2.item(i).text().split(' ')
                f.write(str(self.new_kr.dict[now[0]])+' '+str(self.new_kr.dict[now[1]])+' '+str(now[2])+'\n')
            if self.flag1 and self.flag2 and self.ans1==self.ans2:
                f.write(str(self.ans1)+'\n')
            elif self.ans1!=self.ans2 :
                f.write('算法出错'+'\n')
            elif self.flag1 ==0 and self.flag2==0:
                f.write('构不成最小生成树'+'\n')
            f.write('\n')
